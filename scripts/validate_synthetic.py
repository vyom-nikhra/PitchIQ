...
def align_team_labels(pred_team: np.ndarray, cv_kits: dict, gt_kits: dict) -> np.ndarray:
    """CV clustering picks 'home'/'away' arbitrarily; align to GT by kit colour
    before comparing team-dependent metrics (otherwise a label flip reads as
    catastrophic disagreement)."""
    same = (np.linalg.norm(_hex_rgb(cv_kits["home"]) - _hex_rgb(gt_kits["home"]))
            + np.linalg.norm(_hex_rgb(cv_kits["away"]) - _hex_rgb(gt_kits["away"])))
    flipped = (np.linalg.norm(_hex_rgb(cv_kits["home"]) - _hex_rgb(gt_kits["away"]))
               + np.linalg.norm(_hex_rgb(cv_kits["away"]) - _hex_rgb(gt_kits["home"])))
    if flipped < same:
        swap = {"home": "away", "away": "home"}
        return np.array([swap.get(t, t) for t in pred_team], dtype=object)
    return pred_team
...
def main() -> None:
    ...
    poss_pred = align_team_labels(poss_pred, meta.kit_colors, sim.meta.kit_colors)
...
"""
    ...
    flip = not np.array_equal(
        poss_pred,
        poss.set_index("frame")["team"].reindex(
            range(len(sim.possession_gt))).fillna("none").to_numpy(dtype=object))
