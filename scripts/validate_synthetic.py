...
def foot_boxes(df: pd.DataFrame, w: float = 40.0, h: float = 80.0) -> pd.DataFrame:
...
def teams_flipped_vs_gt(pred: "pd.DataFrame", gt: "pd.DataFrame",
                        every_n: int = 25) -> bool:
    """Is the CV 'home' label actually GT 'away'? Decided by position overlap
    (nearest-GT-player majority vote) — detector-agnostic, unlike kit colours,
    which degrade with poor crops."""
    votes = {"same": 0, "flip": 0}
    gt_by_frame = {f: g for f, g in
                   gt[gt["class"].isin(["player", "goalkeeper"])].groupby("frame")}
    sub = pred[pred.team.isin(["home", "away"])].dropna(subset=["x_pitch", "y_pitch"])
    for f, g in sub.groupby("frame"):
        if f % every_n:
            continue
        ggt = gt_by_frame.get(f)
        if ggt is None:
            continue
        P = g[["x_pitch", "y_pitch"]].to_numpy()
        G = ggt[["x_pitch", "y_pitch"]].to_numpy()
        gt_team = ggt["team"].to_numpy()
        nearest = np.linalg.norm(P[:, None] - G[None], axis=2).argmin(axis=1)
        for cv_t, gt_t in zip(g["team"].to_numpy(), gt_team[nearest]):
            if gt_t not in ("home", "away"):
                continue
            votes["same" if cv_t == gt_t else "flip"] += 1
    return votes["flip"] > votes["same"]


def apply_flip(team_arr: np.ndarray) -> np.ndarray:
    swap = {"home": "away", "away": "home"}
    return np.array([swap.get(t, t) for t in team_arr], dtype=object)
...
def main() -> None:
    ...
    ap.add_argument("--keypoint-weights", default=None,
                    help="validate WITH the learned keypoint calibrator")
    ap.add_argument("--detector-weights", default=None,
                    help="validate WITH the fine-tuned YOLO (default: blob fallback)")
    ...
    detection = ({"backend": "yolo", "weights": args.detector_weights,
                  "device": args.device} if args.detector_weights
                 else {"backend": "blob"})
    overrides = {
        "simulator": {"half_minutes": 1.0, "seed": 21},
        "detection": detection,
        "jersey": {"enabled": False},
        "marking": {"min_defensive_frames": 25},  # 2-min clip
    }
    ...
    # ---------------------------------------------------------- tracking
    # SYMMETRIC foot-point proxy boxes on both sides: true GT box geometry vs
    # fixed pred boxes would turn size mismatch into fake misses
    gt_fb = rr.boxes[rr.boxes["class"].isin(["player", "goalkeeper"])].copy()
    gt_fb["x_pixel"] = (gt_fb.x1 + gt_fb.x2) / 2
    gt_fb["y_pixel"] = gt_fb.y2
    ...
    flip = teams_flipped_vs_gt(pred, sim.tracking)
    if flip:
        poss_pred = apply_flip(poss_pred)
...
"""
    # marking rows respect the label alignment (CV 'home' may be GT 'away')
    cv_of_gt = {"home": "away", "away": "home"} if flip else {"home": "home", "away": "away"}
    for gt_team, gt_scheme in (("home", "zonal/press"), ("away", "man")):
        op = marking.get(cv_of_gt[gt_team], {}).get("open_play", {})
        md += (f"| Marking | GT-{gt_team} man-score (GT scheme: {gt_scheme}) | "
               f"{op.get('team_man_score', 'n/a')} ({op.get('scheme', op.get('note', '?'))}) |\n")
