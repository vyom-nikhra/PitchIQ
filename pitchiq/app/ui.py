...
if choice:
    _sel = ArtifactStore(next(m["dir"] for m in ready if m["id"] == choice))
    if _sel.meta_path.exists():
        _m = _sel.load_meta()
        _kits = "".join(
            f'<span style="display:inline-block;width:11px;height:11px;'
            f'border-radius:50%;background:{_m.kit_colors.get(t, "#888")};'
            f'margin-right:4px;border:1px solid #fff3"></span>'
            for t in ("home", "away"))
        st.sidebar.markdown(
            f'{_kits}<span style="font-size:0.8em;color:gray">'
            f'{_m.n_frames} frames @ {_m.fps:.0f} fps · '
            f'{_m.n_frames / max(_m.fps, 1):.0f}s</span>',
...
if not st.session_state.get("tour_done"):
    with st.expander("👋 New here? 60-second tour", expanded=True):
        st.markdown(
            "**PitchIQ turns one broadcast clip into a tactical analysis.** "
            "Everything below was computed from pixels — no event feed.\n\n"
            "1. **🎬 Annotated video** — every player boxed with a persistent ID "
            "and team colour; the ball tracked through blur and flight.\n"
            "2. **🗺️ Tactical map** — the signature view: a top-down radar in real "
            "metres, frame-synced to the video. Try scrubbing; toggle trails, "
            "team shape and pass arrows.\n"
            "3. **📊 Analytics** — possession, pitch control, pass networks, "
            "formations, xT.\n"
            "4. **🧠 Intelligence** — discovered roles, similar-player search, "
            "who-marks-whom.\n"
            "5. **📝 Report** — an analyst write-up grounded in the computed "
            "numbers, plus free-text Q&A.\n\n"
            "*Numbers come with honesty labels: ground-truth-validated analytics "
            "vs full-CV-pipeline output are kept separate — see the README.*")
...
with tab_radar:
    ...
    events_df = store.load_events() if store.events_path.exists() else None
    with st.spinner("Building radar…"):
        html = build_radar_html(bundle["tracking"], meta, video_for_radar,
                                radar_fps=CFG.app.radar_fps, events=events_df)
    components.html(html, height=1000 if video_for_radar else 660, scrolling=False)
    st.caption("The radar is drawn from projected pitch coordinates and stays "
               ...
               "Toggle motion trails, team-shape hulls and pass arrows below "
               "the pitch.")
...
with tab_analytics:
    ...
    dl1.download_button(
        "⬇️ Tracking table (CSV)",
        bundle["tracking"].to_csv(index=False).encode(),
        file_name=f"{choice}_tracking.csv", mime="text/csv",
        help="Every entity's (x, y) in pitch metres, per frame — the artifact "
             "all analytics derive from.")
    if store.events_path.exists():
        dl2.download_button(
            "⬇️ Events (CSV)",
            store.load_events().to_csv(index=False).encode(),
            file_name=f"{choice}_events.csv", mime="text/csv")
